...
public class Worker extends MessageDelegatingActor implements Requests1<RequestMessage> {
    private LoggingAdapter log = Logging.getLogger(context().system(), this);

    @Override
    public void onRequest(RequestMessage request) {
        log.info("Boss says: " + request.request);
        sender().tell(new ResponseMessage("ok, I'm done"), self());
    }
}
